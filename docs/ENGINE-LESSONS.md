...
| 29 | A demo is ONE paste-and-run file: it carries the libraries it needs, libraries first, collisions refused never merged. | `lib/examples/xtQRdecoder-demo.livecodescript` carries the combined library between sentinels above its own code; `tools/sync_demo_embeds.py` rewrites the copy from the master and refuses a duplicate handler or script-level name. | STATIC | sync --check (mutation-tested) |
| 30 | Demo UI is one carried kit; a look change edits the master and is re-carried, never patched in an adopter. | The showcase carries the suite's UI kit v2 verbatim from `tools/ui-kit.livecodescript`, vendored byte-identical from the suite; the sync check holds the copy. | STATIC | sync --check |
| 31 | Every demo prints its own record: a carried boot self-check with a derived control list, non-destructive, fail loud and pass quiet. | The showcase carries `tools/demo-selfcheck.livecodescript` verbatim; `qdScRun` asserts the library version, every derived control, and four embedded samples decoded to their known texts, then arms the delayed-write probe; the Samples tab is the citable image record. | DESIGN (needs an OXT pass) | sync --check derives the list |
| 32 | A script-only stack file opened from disk builds no window; the ritual is paste, apply, close, reopen. | The showcase's header and `lib/examples/README.md` spell out the five steps and say why. | - | - |
